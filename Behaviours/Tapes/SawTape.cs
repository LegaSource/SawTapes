...
public class SawTape : PhysicsProp
{
    ...
    {
        if (isGameStarted || isGameEnded || isPlayerFinded || miniGameLimit >= ConfigManager.miniGameLimit.Value) return;
    ...
    public void GasPlayersClientRpc()
    {
        ...
        _ = StartCoroutine(GasPlayersCoroutine());
    }
    ...
    public virtual void EndGameForServer(bool isGameCancelled = false)
    {
        ...
        _ = StartCoroutine(SpawnBillyCoroutine(player));
    }

    public virtual bool ExecutePreEndGameActionForServer(bool isGameCancelled) => true;
